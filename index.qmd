---
title: "<img src='C:/Users/Daniel.walsh/Desktop/Branding/Agri-Food-Regulator-Logo-CMYK.svg' style='height: 50px; width:160px; vertical-align: middle; margin-right: 8px; filter: brightness(1.5) contrast(1.1);'><span style='font-size:16px; white-space: nowrap; padding: 6px;'>Ireland's Horticulture Sector: A Comparative View</span>"
format:
  dashboard:
    self-contained: true
...
/* Fix dashboard to your browser viewport size */
.dashboard-page {
  width: 1200px;
  height: 602px;
  max-width: 1200px;
...
/* Center the dashboard on larger screens */
body {
  display: flex;
  min-height: 100vh;
  margin: 0;
...
```{r}
bslib::value_box(
  title = HTML("<span style='font-size:clamp(0.6rem, 2vw,0.85rem); line-height: 1.2;'>Consumer Food Price Increase (2023-2024) - Ireland:</span>"),
  value = HTML("<span style=font-size:2.5rem>6.5%</span>"),
...
bslib::value_box(
  ...
  value = HTML("<span style='font-size:2.5rem'>6.2%</span>"),
  theme =  value_box_theme(
    bg = "#8fb6df",
    fg = "white"
  ),
...
bslib::value_box(
  ...
  value = HTML("<span style='font-size:2.5rem'>3.3%</span>"),
  theme =  value_box_theme(
    bg = "#3c5268",
    fg = "white"
  ),
...
ggplot(fruit_comparison, aes(x = date, y = value, group = country)) +
  ...
  geom_line_interactive(data = fruit_comparison %>% filter(country != c("Ireland", "EU27")),
            color = "#8fb6df",
            alpha = 0.15,
            linewidth = 0.5) +
              
   geom_line_interactive(data = fruit_comparison %>% filter(country == "EU27") ,
            color = "#3c5268",
            alpha = 0.5,
            linewidth = 0.75) +
  ...
  geom_point_interactive(data = fruit_comparison %>%  filter(country == "Ireland"),
   ...
   geom_point_interactive(data = fruit_comparison %>%  filter(country == "EU27"),
             color = "#3c5268",
             size = 2) +
...
```{r}
fruit_comparison_annual <- read_excel("C:\\Users\\Daniel.walsh\\Desktop\\Horticulture\\Horticulture\\Projects\\01_horticulture_public_data_projects\\00_horticulture_public_data_report_2025\\02_processed_data\\00_cleaned_data\\00_price_data\\00_overall\\00_consumer_indices\\hicp_annual.xlsx") 


fruit_comparison_annual <- fruit_comparison_annual %>% 
  filter(
    category %in% c("Fruit"),
    country %in% c("Ireland", "EU27")
  )
...
last_values_fruit_annual <- fruit_comparison_annual %>% 
  group_by(country) %>% 
  slice_max(date) %>% 
  ungroup
...
fruit_hicp_comparison_annual <-  
  
  ggplot(fruit_comparison_annual, aes(x = date, y = value, color = country)) +
  
  geom_line_interactive(linewidth = 0.75) +
  geom_point_interactive(aes(shape = country), size = 2) +
  geom_text(data = last_values_fruit_annual, aes(label = country),
            size = 2.5,
            vjust = 2) +
  scale_shape_manual(values = c("Ireland" = 15, "EU27" = 16)) +
  scale_color_manual( values = c("Ireland" = "#8fb6df", "EU27" = "#3c5268")) + 
  ...
  labs(
    title = "Better Deal for Irish Shoppers: EU Fruit Prices Have Risen More Rapidly Than at \n Irish Checkouts",
    subtitle = "**Annual Fruit Price Index Trends:**
     <span style='color:#8fb6df;'>**Ireland**</span>,
    <span style='color:#3c5268;'>**EU27**</span> (2015-2024). The graph shows growth in consumer prices for fruit over the period 2015-2024. Irish <br>prices in supermarkets have seen significantly less growth than the EU-27 average, with potential implications for growers. Consumer prices for fruit <br>rose 10% in Ireland since 2015, compared to EU-27 growth of 43%",
    caption = "Source: Eurostat
    <br>Notes : Base year is 2020. Monthly data should be interpreted with caution given differneces in seasonality in fruit production acroos the EU-27."
  ) +
...
fruit_hicp_comparison_interactive_annual <- girafe(ggobj = fruit_hicp_comparison_annual,
                                            options = list(
                                              opts_hover(css = hover_css),
                                              opts_hover_inv(css = "opacity:0.02"),
                                              opts_tooltip(
...
fruit_hicp_comparison_interactive_annual
...
```{r}
combined_data <- read_excel("C:\\Users\\Daniel.walsh\\Desktop\\Horticulture\\Horticulture\\Projects\\01_horticulture_public_data_projects\\00_horticulture_public_data_report_2025\\02_processed_data\\00_cleaned_data\\00_price_data\\00_overall\\00_consumer_indices\\hicp_annual.xlsx")
```

```{r}
annual_inflation <- combined_data %>% 
...
diff_annual_inflation <- annual_inflation %>% 
...
diff_annual_inflation_fruit <- diff_annual_inflation %>%
  filter(category == "Fruit") %>%
  ggplot(
  ...
  theme_minimal() +
  labs(
    title = "Measuring the Gap: Exactly How Much Irish Consumer Price Inflation For Fruit \nHas Differed From the EU-27 ",
    subtitle = "**Difference Between Irish and EU-27 Annual Inflation Rates:** The graph measures the absolute gap between
    Irish consumer fruit price growth <br>and EU-27 consumer fruit price growth, in percentage points. Irish fruit price growth at the supermarket has remained below EU-27 rates in 8 out <br>of the 9 years since 2015, averaging . Only 2019 showed higher inflation in Ireland.",
...
diff_annual_inflation_fruit_interactive <- girafe(ggobj = diff_annual_inflation_fruit,
                               options = list(
                                  opts_hover(css = hover_css),
                                  opts_hover_inv(css = "opacity:0.02"),
                                  opts_tooltip(
...
diff_annual_inflation_fruit_interactive
...
```{r}
bslib::value_box(
  title = HTML("<span style='font-size:clamp(0.6rem, 2vw,0.85rem); line-height: 1.2;'>Imports:</span>"),
  value = HTML("<span style=font-size:2.5rem>34.8%</span>"),
  subtitle = HTML("<span style='font-size:0.75rem'>Input cost growth was 34.8% points bigger than producer price growth (2020-2023)</span>"),
  theme =  value_box_theme(
    bg = "#8fb6df",
    fg = "white"
  ),
...
```{r}
bslib::value_box(
  title = HTML("<span style='font-size:1rem'>Correlation Between Producer Prices and Consumer Prices:</span><span style='font-size:0.75rem;'>Cumulative growth in producer prices in Ireland was 22.3 % points below the EU27  (2015-2023)"),
  value = "",
...
```{r}
bslib::value_box(
  title = HTML("<span style='font-size:clamp(0.75rem, 2vw, 1rem); line-height: 0.5;'>Relative Cumulative Growth - Fruit:</span>"),
  value = HTML("<span style=font-size:2rem>22.3%</span><span style='font-size: 0.7rem; opacity: 0.85;'>Cumulative growth in producer prices was 22.3% points below the EU27 over the period 2015-2023,</span>"),
  theme =  value_box_theme(
    bg = "#8fb6df",
    fg = "white"
  ),
...
```{r}
bslib::value_box(
  title = HTML("<span style='font-size:clamp(0.75rem, 2vw, 1rem); line-height: 0.5;'>Squezed Margin Growth - Fruit:</span>"),
  value = HTML("<span style=font-size:2rem>34.8%</span><span style='font-size: 0.7rem; opacity: 0.85;'>Producer price growth was 34.8% points below input cost growth over the period 2020-2023,</span>"),
  theme =  value_box_theme(
    bg = "#3c5268",
    fg = "white"
  ),
...
```{r}
bslib::value_box(
  title = HTML("<span style='font-size:1rem'>Squeezed Margin Growth - Veg:</span><span style='font-size:0.85rem'>Input cost growth was 29.24 % points bigger than producer price growth (2020-2023)"),
  value = "",
  subtitle = HTML("<span style='font-size:0.75rem'>Input Cost Growth vs Producer Price Growth (2020-2023)</span>"),
  theme =  value_box_theme(
    bg = "#3c5268",
...
```{r}
bslib::value_box(
  title = HTML("<span style='font-size:1rem'>Relative Cumulative Growth - Veg:</span><span style='font-size:0.85rem'>Cumulative growth in producer prices in Ireland was 19.8 % points below the EU27 (2015-2023)"),
  value = "",
  subtitle = HTML("<span style='font-size:0.75rem'>Input Cost Growth vs Producer Price Growth (2020-2023)</span>"),
  theme =  value_box_theme(
    bg = "#3c5268",
